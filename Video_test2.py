...
cap = cv2.VideoCapture("data/video/11.mp4")
def video():
    while (cap.isOpened()):
        tongframe = 0
        biensotimthay = 0
        ...
        if detected == 1:
            ...
            for screenCnt in screenCnt:
                ...
                if len(char_x) in range(7, 10):
                    ...
                    print("\n Bien so xe: " + strFinalString + "\n")
                    # label = Label( text="Kết quả:                                      "
                    #             "                                              "
                    #             "                                              "
                    #             "                                              ", bg='white', fg='black', font=('Times 26 bold'))
                    # label.place(x=1040, y=600)
                    # label2 = Label( text=str(first_line+second_line), bg='white', fg='black', font=('Times 28 bold'))
                    # label2.place(x=1185, y=600)
                    
                    cv2.putText(img, strFinalString, (topy, topx), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 255), 1)
                    n = n + 1
                    biensotimthay = biensotimthay + 1

                    cv2.imshow("Bien so", cv2.cvtColor(roi, cv2.COLOR_BGR2RGB))
...
def camera():
    while (cap1.isOpened()):
        ...
        screenCnt = []
        for c in contours:
            peri = cv2.arcLength(c, True)  # Tinh cv
                ...
                if len(char_x) in range(7, 10):
                    ...
                    for i in char_x:
                        ...
                        npaROIResized = imgROIResized.reshape(
                            (1, RESIZED_IMAGE_WIDTH * RESIZED_IMAGE_HEIGHT))  # dua anh ve mang 1 chieu
                        # chuyen anh ve dang ma tran
                        npaROIResized = np.float32(npaROIResized)  # chuyen float
                        _, npaResults, neigh_resp, dists = kNearest.findNearest(npaROIResized,k=3)  # call KNN function find_nearest; neigh_resp là hàng xóm
                        strCurrentChar = str(chr(int(npaResults[0][0])))  # ASCII 
                        cv2.putText(roi, strCurrentChar, (x, y + 50), cv2.FONT_HERSHEY_DUPLEX, 2, (0, 255, 255), 3)
...
# thay đổi màu nền của giao diện
root.configure(bg='#F5F5F5')

# tạo các nút bấm và đặt vị trí
bt = Button(root, text="Quit", font=("Arial", 14), fg="white", bg="#FF5733", width=30, command=quit)
bt.place(x=50, y=150)

bt1 = Button(root, text="Video", font=("Arial", 14), fg="white", bg="#C70039", width=30, command=video)
bt1.place(x=50, y=0)

bt2 = Button(root, text="Image", font=("Arial", 14), fg="white", bg="#900C3F", width=30, command=image)
bt2.place(x=50, y=50)

bt3 = Button(root, text="Camera", font=("Arial", 14), fg="white", bg="#581845", width=30, command=camera)
